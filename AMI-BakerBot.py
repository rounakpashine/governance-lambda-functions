...
#Description: This lambda function creates the backup AMI for EC2 instances with tag "AMIBackUp=Yes".
...
def setGlobalVars(event):
    ...
    try:
        globalVars['REGION_NAME'] = event.get('REGION_NAME', "us-east-2")
    ...
    for instance in instances:
        ...
        try:
            response = ec2_client.create_image(InstanceId=instance['InstanceId'],
                                               Name=NameTxt,
                                               Description='AMI-for-' + str(
                                                   instance['InstanceId']) + '-' + datetime.datetime.now().strftime(
                                                   '%Y-%m-%d_%-H-%M'),
        ...
        logger.info("Instance-id=" + instance['InstanceId'] + " Image-id=" + response[
            'ImageId'] + " Deletion Date=" + temp_delete_fmt)
...
def push_to_sns(imagesBaked):
    ...
    try:
        ...
        logger.error(
            'ERROR: Unable to push to SNS Topic: Check [1] SNS Topic ARN is invalid, [2] IAM Role Permissions{0}'.format(
                str(e)))
